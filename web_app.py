...
VISUALIZATION_TITLES = {
    ...
    "growth_trend": "可比性与变化状态分布",
    "risk_change": "已验证可比记录风险等级变化",
    "top_growth": "可比跨巡检面积审计 Top 10",
    "disease_type": "病害类型分布",
...
def _load_csv_payload(path: Path, missing_key: str, fields: list[str] | None = None, row_projector=None) -> dict:
    errors = {}
    limitations = []
    rows = _read_csv_rows(path, missing_key, errors, limitations)
    if row_projector is not None:
        rows = [row_projector(row) for row in rows]
...
def _project_credibility_row(row: dict, priority_output: bool = False) -> dict:
    """Return a Web-safe copy without directional claims from unverified rows."""

    item = dict(row)
    verified = item.get("comparability_status") == "verified_comparable"
    item["directional_metrics_available"] = verified
    item["descriptive_area_relative_difference"] = ""
    item["descriptive_area_difference_px"] = ""
    if verified:
        return item

    item["descriptive_area_relative_difference"] = item.get("area_growth_rate", "")
    item["descriptive_area_difference_px"] = item.get("area_growth_px", "")
    item["growth_trend"] = (
        "不可比较"
        if priority_output or item.get("comparability_status") != "insufficient_history"
        else "数据不足"
    )
    item["growth_description"] = (
        f"静态面积审计：{item.get('first_area_px', '')} px² -> "
        f"{item.get('last_area_px', '')} px²。当前记录不构成方向性变化结论。"
    )
    if priority_output:
        item["recheck_reason"] = "当前关注等级或风险等级满足复检条件；纵向可比性尚未验证。"
        item["recheck_suggestion"] = "建议记录静态面积证据，并在取得真实复检数据后再判断方向性变化。"
    item["area_growth_rate"] = ""
    item["area_growth_px"] = ""
    item["risk_level_change"] = ""
    item["first_risk_level"] = ""
    return item
...
def _load_project_summary() -> dict:
    ...
    obvious_growth_count = sum(
        1
        for row in growth_rows
        if row.get("comparability_status") == "verified_comparable"
        and row.get("growth_trend") in {"明显增长", "持续增长"}
    )
    missing = [
        str(path).replace("\\", "/")
...
GROWTH_ANALYSIS_FIELDS = [
    ...
    "directional_metrics_available",
    "descriptive_area_relative_difference",
    "descriptive_area_difference_px",
]
RECHECK_FIELDS = [
    "priority_rank",
    "disease_id",
...
def _load_robot_dashboard() -> dict:
    ...
    priority_rechecks = [
        _project_credibility_row(row, priority_output=True)
        for row in _read_csv_rows(PRIORITY_RECHECK_FILE, "priority_recheck_list", errors, limitations)
    ]
    growth_rows = [
        _project_credibility_row(row)
        for row in _read_csv_rows(DISEASE_GROWTH_FILE, "disease_growth_analysis", errors, limitations)
    ]
    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/growth-analysis":
            self._send_json(
                _load_csv_payload(
                    DISEASE_GROWTH_FILE,
                    "disease_growth_analysis",
                    GROWTH_ANALYSIS_FIELDS,
                    _project_credibility_row,
                )
            )
            return
        if path == "/api/recheck-list":
            self._send_json(
                _load_csv_payload(
                    PRIORITY_RECHECK_FILE,
                    "priority_recheck_list",
                    RECHECK_FIELDS,
                    lambda row: _project_credibility_row(row, priority_output=True),
                )
            )
            return
