...
def score_review_priority(
    ...
    if unc.get("available") is False:
        ...
        if defect_mean is not None and float(defect_mean) >= cfg.uncertainty_review:
            score += 1.0
            reasons.append(f"defect uncertainty {float(defect_mean):.3f} exceeds review threshold {cfg.uncertainty_review:.2f}")
    ...
    if dis.get("available") is False:
        ...
        if defect_disagreement is not None and float(defect_disagreement) >= cfg.disagreement_review:
            score += 0.75
            reasons.append(f"TTA disagreement {float(defect_disagreement):.3f} indicates unstable prediction")
    ...
    if foreground_iou is not None:
        ...
        if foreground_iou < cfg.severe_self_consistency:
            score += 1.25
            reasons.append(f"single/fused foreground IoU {foreground_iou:.3f} is severely unstable")
        elif foreground_iou < cfg.low_self_consistency:
            score += 0.75
            reasons.append(f"single/fused foreground IoU {foreground_iou:.3f} is below stability threshold")
    ...
    if single_fg > 0:
        ...
        if fused_ratio < cfg.shrink_review_ratio:
            score += 0.75
            reasons.append(f"fused foreground shrinkage ratio {fused_ratio:.3f} may suppress small defect evidence")
    ...
    if selection_mode in {"single", "hybrid"}:
        score += 0.5
        reasons.append(f"adaptive selection chose {selection_mode} instead of fixed fused output")
