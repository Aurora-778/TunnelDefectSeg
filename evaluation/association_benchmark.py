...
def write_benchmark_outputs(fixture: dict[str, Any], output_dir: Path) -> dict[str, Path]:
    ...
    lines.extend([
        ...
        "- conflict_count 仅表示单个 query 内存在至少两个候选且分数差小于生产 margin 阈值的候选歧义；reject 与单候选不计入。",
        "- conflict_count 不表示全局 one-to-one 分配冲突，也不表示不同 query 之间的竞争分配。",
        "- 当前 benchmark 不能证明真实机器人连续巡检中的长期泛化能力，也不能替代带跨巡检 GT 的现场验证。", "",
        "## 重复命中边界", "",
        "C11/C12 是两个独立逐帧 query 对同一历史对象的重复命中审计。当前 benchmark 对每个 query 独立评估，不执行 query 之间的全局 one-to-one assignment，也不声称已经解决多目标唯一分配问题。", "",
        "## 自动接受案例", "",
        ...
        f"在当前 benchmark 上，weighted_no_id 的 top1 accuracy 未优于 spatial_only（{weighted['top1_accuracy']} vs {spatial['top1_accuracy']}）。",
        "weighted_no_id 的价值主要体现在多维证据记录、拒绝与人工复核边界，以及后续真实数据扩展能力；当前结果不足以证明其准确率优于简单空间基线。",
        "当前 benchmark 规模不足以支持泛化结论，不包含全局 one-to-one assignment；with_id_upper_bound 不是可部署 baseline。", "",
        "## 策略说明", "",
