...
本文是研发侧权利要求草稿，用于把当前算法链转成可讨论的专利表达。正式申请前需要由专利代理人根据查新结果、现有技术和保护范围进行法律化改写。

核心保护对象是 post-inference confidence review 方法，不是 SegFormer、TTA、entropy uncertainty、skeletonization 或 mIoU 指标本身。
...
1. 一种隧道病害分割结果可信复核方法，其特征在于，包括：
   ...
   4. 比较单次分割候选结果和融合分割候选结果的前景面积、一致性和前景保护像素，识别固定融合对病害区域的压缩或丢失；
...
本草稿的保护重点是候选分割结果的可信复核与证据生成链条。以下内容不应作为独立发明点主张：
