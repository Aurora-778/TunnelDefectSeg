...
| C2 | Fixed fusion can reduce or harm defect foreground on labelled samples. | supported | `experiments/enhancement_evidence_test_summary.json`; `experiments/enhancement_evidence_all_summary.json` | GT-required aggregate evidence. |
...
| C4 | Adaptive selection protects foreground pixels relative to fixed fused output. | supported | `small_defect_guard` in evidence summaries; C1 in `docs/experiments/patent-case-pack.md` | Depends on current thresholds and labelled evaluation. |
| C5 | Uncertainty is useful as a review signal on labelled data. | supported | `uncertainty_review` in enhancement summaries; C3 in `docs/experiments/patent-case-pack.md` | Error overlap and calibration require GT. |
