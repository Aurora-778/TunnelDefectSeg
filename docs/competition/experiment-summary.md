...
| Spatial mapping prototype | Implemented prototype | Current code can emit `location_source`, `accuracy_level`, `clock_position`, `ring_id`, `mileage`, and optional `local_3d`. Evidence still needs to be labelled as simulation/calibration until field data is available. |
...
4. Spatial mapping is now implemented as a prototype, but its evidence boundary is still simulation/calibration until field data is available.
...
- Finally show report export as the current system-level engineering closure, and show spatial mapping as an implemented prototype with explicit simulation/calibration boundaries.
