...
| R4 | 每个结果都要包含类型、置信度、几何和空间位置等信息 | planned | `docs/competition/report-template.md` | 需要把报告字段真正接到多领域 schema 和空间定位模块。 |
| R5 | 像素坐标要能映射到里程、环号、时钟方位和局部三维坐标 | simulation-only | `docs/plans/2026-06-18-001-feat-metro-multidomain-inspection-plan.md` | 目前只能作为仿真或标定方向展示，不能写成已完成外业定位。 |
| R6 | 空间映射必须区分真实传感器、仿真输入和手动标定输入 | planned | `docs/competition/experiment-summary.md`; `docs/competition/demo-case-pack.md` | 需要给每个 location 结果补 `location_source` 和 `accuracy_level`。 |
| R7 | RGB、深度、点云、内参、外参、姿态和里程信息要留扩展接口 | planned | `docs/plans/2026-06-18-001-feat-metro-multidomain-inspection-plan.md` | 先保留接口，不把多模态融合写死在单一模型里。 |
| R8 | 无点云时允许用隧道圆筒/环片参数做仿真定位 | simulation-only | `docs/plans/2026-06-18-001-feat-metro-multidomain-inspection-plan.md` | 需要明确标注定位来源和精度等级，避免把估算写成实测。 |
...
| R11 | Web 展示要支持按领域、病害类型、位置和复核优先级浏览 | planned | `web_app.py`; `web_demo/index.html` | 需要把当前单图浏览升级成比赛视角的筛选/分组入口。 |
...
- 没有官方样本或真实传感器数据时，任何定位或评分都必须明确标注为 simulation-only 或 planned。
