...
8. 受控工作流只在隔离临时沙箱中按固定任务契约运行；发生异常或冲突提示时，应阻断任务并保留证据，交由受控维护流程处理，CLI 不提供普通用户恢复命令。
9. 输出报告可用于项目展示和材料整理，但正式工程应用前需要人工复核和专业检测流程确认。
10. R11 登记表“软件的技术特点”（100字内）可填写：人工智能辅助分析软件，支持单sequence、已有mask的离线校验、病害特征提取、路线关联和复检提示；在隔离临时沙箱内形成离线工程原型处理闭环，本地Web仅展示已生成结果。
