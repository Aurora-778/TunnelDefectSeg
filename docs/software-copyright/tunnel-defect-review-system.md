...
本软件面向隧道巡检图像的病害语义分割、可信复核和可视化展示。系统以图像为输入，输出病害区域 `mask`、叠加图、uncertainty、disagreement、骨架、形态量化指标、风险提示和人工复核优先级。
...
第三方依赖仅作为运行框架或模型组件使用，不应在软著材料中描述为本软件自研源码。
