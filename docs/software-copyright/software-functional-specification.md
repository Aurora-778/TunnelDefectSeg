...
本软件面向机器人隧道巡检病害分析场景，提供从巡检图像、已有 mask 和工程元数据的受控接入，到病害几何信息提取、病害对象记忆、无标识关联、路线级时空追踪、比较证据与声明门控、复检清单、受控任务执行、视频展示和 Web 结果查看的工程原型流程。
...
7. Web 展示层：通过本地 Web Dashboard 展示系统总览、工程报告、路线级报告、规则面积变化提示、复检清单、可视化图表、单图复核结果和视频分析结果。
...
该模块通过本地 Web 页面展示系统总览、工程报告、路线级报告、规则面积变化提示、重点复检清单、可视化图表、单图检测 / 复核结果和视频分析结果。当前 Web 主要用于展示已经生成的 demo 分析结果，不执行受控工作流。
...
真实巡检输入适配模块校验单 sequence 的图像、已有 mask 和工程元数据，生成观测记录、下游兼容的帧记录和完整性清单，并在巡检次数不足或有效观测为空时明确标记为不可推理。受控工作流模块仅在隔离临时沙箱中运行固定任务契约：它保存输入快照、任务状态、操作日志、运行锁和发布清单，并在异常后通过受控恢复路径继续或阻断任务。该模块不提供 Web 在线执行、任务队列服务或生产部署能力。
...
- 路线级时空报告、defect track 和带声明边界的复检队列；
