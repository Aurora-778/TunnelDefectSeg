...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export video from supervision annotated frames.")
...
def load_cv2():
    try:
        ...
        raise RuntimeError("OpenCV is required to export supervision annotated video. Please install opencv-python.") from exc
    return cv2
...
def default_paths(video_id: str) -> tuple[Path, Path]:
    ...
    return base_output / "supervision_annotated_frames", base_output / "supervision_annotated_video.mp4"


def collect_frames(frames_dir: Path) -> list[Path]:
    if not frames_dir.is_dir():
        raise FileNotFoundError(f"supervision annotated frames_dir not found: {frames_dir}")
    frames = sorted(frames_dir.glob("frame_*.jpg"))
    if not frames:
        raise ValueError(f"no supervision annotated frame_*.jpg files found in: {frames_dir}")
    return frames
...
def export_supervision_annotated_video(
    video_id: str,
    ...
    try:
        for frame_path in frames:
            ...
            if frame is None:
                raise ValueError(f"unable to read supervision annotated frame: {frame_path}")
...
def main() -> None:
    args = parse_args()
    try:
        output_video = export_supervision_annotated_video(
    ...
    print("supervision annotated video export completed")
